...
func TestStorage_StartClean(t *testing.T) {
	rows, _ := s.conn.Query(s.ctx, "DELETE FROM charms;")
	rows.Close()
	rows, _ = s.conn.Query(s.ctx, "ALTER SEQUENCE charms_id_seq RESTART WITH 1;")
	rows.Close()
}
